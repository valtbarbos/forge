...
export class Node extends GObject.Object {
  ...
  /**
   * Serialize this node to a plain JSON-serializable object.
   * Used to persist the tree state to disk.
   */
  toSerializable() {
    const base = {
      type: this.nodeType,
    ...
    // Non-window nodes have stable ids (workspace, monitor, container)
    if (!this.isWindow()) {
      if (typeof this.nodeValue === "string") {
        base.id = this.nodeValue;
      }
      if (this.layout) {
        base.layout = this.layout;
      }
      if (typeof this.percent === "number" && this.percent > 0) {
        base.percent = this.percent;
      }
      ...
      if (this.childNodes && this.childNodes.length > 0) {
        ...
        for (const child of this.childNodes) {
          ...
          if (serialized) {
            base.children.push(serialized);
          }
        }
      }
    } else {
      // Window node: we store lightweight meta info
      ...
      try {
        base.wmId = metaWindow.get_id();
        base.mode = this.mode;
        if (typeof this.percent === "number" && this.percent > 0) {
          base.percent = this.percent;
        }
        base.float = this.isFloat();

        // Optional fuzzy matching
        base.meta = {
          wmClass: metaWindow.get_wm_class(),
          wmTitle: metaWindow.get_title(),
        };
      } catch (e) {
        // Window may have been destroyed
        Logger.warn(`Failed to serialize window node: ${e}`);
        return null;
      }
    }
...
export class Tree extends Node {
  ...
  /**
   * Serialize the current tree state into a plain JSON object.
   * This is the single source of truth for layout persisted on disk.
   */
  toJSONTree() {
    const rootSerializable = this.toSerializable();
    return {
      version: 1,
  ...
  /**
   * Reload the tree layout from a serialized JSON state.
   *
   * Strategy:
   *  - Re-initialize workspaces/monitors as usual (_initWorkspaces).
   *  - Use the JSON tree to:
   *      * restore container layouts and percents
   *      * re-order / re-parent windows by best-effort matching (wmId, class/title)
   *  - If anything fails, we fall back to the live tracking in WindowManager.
   *
   * @param {object} treeState Parsed JSON from tree.json
   * @param {Meta.Window[]} windowsAll List of windows across all workspaces
   * @returns {boolean} true if successfully reloaded from JSON, false otherwise
   */
  reloadFromJson(treeState, windowsAll) {
    if (!treeState || !treeState.root) {
      return false;
    }

    // Check version compatibility
    if (treeState.version !== 1) {
      Logger.warn(`Unsupported tree state version: ${treeState.version}`);
      return false;
    }

    // Clear existing layout under ROOT
    this.childNodes.length = 0;
    this.attachNode = undefined;

    // Rebuild basic workspace/monitor skeleton so actorBins & signals exist
    this._initWorkspaces();

    // Build a lookup for windows by id + class/title
    const byId = new Map();
    ...
    for (const win of windowsAll) {
      try {
        ...
        const klass = win.get_wm_class();
        const title = win.get_title();
        const key = `${klass}::${title}`;
        if (!byClass.has(key)) {
          byClass.set(key, []);
        }
        byClass.get(key).push(win);
      } catch (e) {
        // Ignore odd windows
      }
    }

    // Helper to resolve a window from JSON
    const resolvedWindows = new Set();

    const resolveWindow = (nodeJson, workspaceIndexHint) => {
      if (!nodeJson) return null;

      // Try to match by window ID first
      if (typeof nodeJson.wmId === "number" && byId.has(nodeJson.wmId)) {
        const win = byId.get(nodeJson.wmId);
        resolvedWindows.add(win);
        return win;
      }

      // Fall back to matching by class + title
      const meta = nodeJson.meta || {};
      const klass = meta.wmClass;
      const title = meta.wmTitle;
      if (klass && title) {
        const key = `${klass}::${title}`;
        const candidates = (byClass.get(key) || []).filter((w) => !resolvedWindows.has(w));
      ...
      // Fallback: any unused window on the hinted workspace
      if (typeof workspaceIndexHint === "number") {
        const candidates = windowsAll.filter((w) => {
          if (resolvedWindows.has(w)) return false;
          try {
            const ws = w.get_workspace();
            return ws && ws.index() === workspaceIndexHint;
          } catch (e) {
            return false;
          }
        });
    ...
    // Recursive rebuild under existing workspace/monitor skeleton
    const rebuildNodeChildren = (parentNode, jsonNode, workspaceIndexHint) => {
      if (!jsonNode.children || jsonNode.children.length === 0) {
        return;
      }

      for (const childJson of jsonNode.children) {
        if (childJson.type === NODE_TYPES.WINDOW) {
          const metaWin = resolveWindow(childJson, workspaceIndexHint);
          if (!metaWin) continue;

          const nodeWindow = this.createNode(parentNode.nodeValue, NODE_TYPES.WINDOW, metaWin);
          if (!nodeWindow) continue;

          // Restore mode/percent/float if present
          if (childJson.mode) {
            nodeWindow.mode = childJson.mode;
          }
          if (typeof childJson.percent === "number") {
            nodeWindow.percent = childJson.percent;
          }
          if (childJson.float === true) {
            nodeWindow.float = true;
          }
        } else {
          // Non-window nodes: find or create by id (workspace, monitor, con)
          let childNode = null;

          if (childJson.id) {
            childNode = this.findNode(childJson.id);
          }

          if (!childNode) {
            // For containers we use createNode with parentNode.nodeValue
            const nodeValue = childJson.id || null;
            // Only create CON nodes; workspace/monitor should already exist from _initWorkspaces
            if (childJson.type === NODE_TYPES.CON) {
              const container = new St.Bin();
              childNode = new Node(NODE_TYPES.CON, container);
              childNode.settings = this.settings;
          ...
          if (childJson.layout) {
            childNode.layout = childJson.layout;
          }
          if (typeof childJson.percent === "number") {
            childNode.percent = childJson.percent;
          }

          let nextWorkspaceIndexHint = workspaceIndexHint;
          if (childJson.type === NODE_TYPES.WORKSPACE && typeof childJson.id === "string") {
            const wsIndex = parseInt(childJson.id.replace("ws", ""), 10);
            if (!isNaN(wsIndex)) nextWorkspaceIndexHint = wsIndex;
          }

          rebuildNodeChildren(childNode, childJson, nextWorkspaceIndexHint);
        }
      }
    };

    try {
      const rootJson = treeState.root;
      // ROOT is this
      if (rootJson.layout) {
        this.layout = rootJson.layout;
      }
      rebuildNodeChildren(this, rootJson, null);
      return true;
    } catch (e) {
  ...
  focus(node, direction) {
    ...
    switch (type) {
      ...
      case NODE_TYPES.CON:
        ...
        break;
      case NODE_TYPES.MONITOR:
        // Check if monitor is empty before drilling down
        const monitorWindows = next.getNodeByType(NODE_TYPES.WINDOW);

        if (monitorWindows.length === 0) {
          // EMPTY MONITOR CASE: Warp pointer to center instead of activating window
          const monitorIndex = Utils.monitorIndex(next.nodeValue);
          ...
          Logger.info(`[focus] Warped pointer to empty monitor ${monitorIndex} at (${rect.x + Math.floor(rect.width / 2)}, ${rect.y + Math.floor(rect.height / 2)})`);
          return next; // Successfully focused the empty monitor via pointer
        }

        // Standard drill-down logic for non-empty monitors
        if (next.layout === LAYOUT_TYPES.STACKED) {
          next = next.lastChild;
        } else {
          if (previous) {
    ...
    if (metaWindow.minimized) {
      ...
      if (this.settings.get_boolean("move-pointer-focus-enabled")) {
        // Pointer follows focus: always warp to the center of the new window
        this.extWm.movePointerWith(next, { center: true });
      } else if (!pointerInside) {
        // Even with the feature off, when we do warp (e.g. cross-monitor),
        // warp to center instead of top
        this.extWm.movePointerWith(next, { force: monitorChanged, center: true });
      }
    }
    return next;
  }
  ...
  next(node, direction) {
    ...
    switch (type) {
      ...
      case NODE_TYPES.WORKSPACE:
        ...
        break;
      case NODE_TYPES.MONITOR:
        // Find the next monitor - handle empty monitors by using node ID directly
        const nodeWindow = this.findFirstNodeWindowFrom(node);
        if (nodeWindow) {
          return this.nextMonitor(nodeWindow, position, orientation);
        } else {
          // Empty monitor case: extract monitor index from node ID
          const currentMonitorIndex = Utils.monitorIndex(node.nodeValue);
          const monitorDirection = Utils.directionFrom(position, orientation);
          const targetMonitorIdx = global.display.get_monitor_neighbor_index(
            currentMonitorIndex,
            monitorDirection
          );

          if (targetMonitorIdx >= 0) {
            // Extract workspace from current node ID (e.g., "mo1ws0" -> "0")
            const wsIndex = node.nodeValue.split('ws')[1];
            const targetMonWs = `mo${targetMonitorIdx}ws${wsIndex}`;
            Logger.info(`[next] Empty monitor navigation: ${node.nodeValue} -> ${targetMonWs}`);
            return this.findNode(targetMonWs);
          }
          return null;
        }
    }
  ...
  debugNode(node) {
    ...
    Logger.debug(
      `${spacing}${rootSpacing}${dashes} ${node.nodeType}#${node.index !== null ? node.index : "-"
      } @${attributes}`
    );
  }
