...
export class WindowManager extends GObject.Object {
   ...
   *
   * First tries to reload the tree from the JSON dump file (tree.json),
   * which is the single source of truth for the layout.
   * If that fails, falls back to rebuilding from the live Meta.Window list.
   */
  reloadTree(from) {
    if (!this._reloadTreeSrcId) {
      this._reloadTreeSrcId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
        ...
        try {
          ...
          if (state) {
            ...
            if (reloadedFromJson) {
              Logger.info(`Reloaded tree from JSON state`);
              // After JSON reload, track any windows not in JSON (newly opened)
              this.trackCurrentWindows();
            }
          }
        } catch (e) {
        ...
        if (!reloadedFromJson) {
          // Fallback: current behaviour based on live windows & workspaces
          Logger.info(`Reloading tree from live state (JSON reload failed or unavailable)`);
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // Use pointer-based monitor detection for proper Wayland support.
        ...
        const activeMonitor = this._getMonitorIndexAtPointer();
        ...
        if (metaWindow.get_monitor() !== activeMonitor) {
          metaWindow.move_to_monitor(activeMonitor);
        }

        Logger.info(`[trackWindow] Window: ${metaWindow.get_title()}`);
        Logger.info(`[trackWindow] Pointer monitor: ${activeMonitor}, Focus monitor: ${focusMonitor}, Workspace: ${activeWorkspace}`);
        ...
        // Mark this window as placed via pointer-based monitor detection.
        // This prevents window-entered-monitor from overriding our placement.
        metaWindow._forgePointerPlaced = true;
        metaWindow._forgeTargetMonitor = activeMonitor;
        Logger.info(`[trackWindow] Marked window with _forgePointerPlaced=true, targetMonitor=${activeMonitor}`);
