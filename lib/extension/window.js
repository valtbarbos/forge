...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    super();
    this.ext = ext;
    this.prefsTitle = `Forge ${_("Settings")} - ${!production ? "DEV" : `${PACKAGE_VERSION}-${ext.metadata.version}`
      }`;
    ...
    // When true, trackWindow() will *adopt* existing monitor/workspace
    // assignments instead of using pointer-based placement.
    this._adoptLiveLayoutMode = false;
  ...
  _bindSignals() {
    ...
    this._displaySignals = [
      ...
      display.connect("window-entered-monitor", (_, monitor, metaWindow) => {
        // Ignore monitor signals during suspend to preserve layout state
        if (this._isSuspended) {
          Logger.info(`[window-entered-monitor] SUSPENDED - ignoring signal for: ${metaWindow.get_title()}`);
          return;
        }
        // Skip monitor reassignment for windows we placed via pointer detection.
        // The window manager may emit this signal with the "wrong" monitor,
        // but we want to keep our pointer-based placement.
        if (metaWindow._forgePointerPlaced) {
          Logger.info(`[window-entered-monitor] Skipping for pointer-placed window: ${metaWindow.get_title()}, WM monitor=${monitor}, our target=${metaWindow._forgeTargetMonitor}`);
          // Clear the flag after first signal to allow future moves
          delete metaWindow._forgePointerPlaced;
          delete metaWindow._forgeTargetMonitor;
      ...
      display.connect("workareas-changed", (_display) => {
        // Ignore workarea changes during suspend to preserve layout state
        if (this._isSuspended) {
          Logger.info(`[workareas-changed] SUSPENDED - ignoring signal`);
          return;
        }
  ...
  command(action) {
    ...
    switch (action.name) {
      ...
      case "Focus":
        ...
        // Get the monitor index under the pointer (Wayland-aware)
        const pointerMonitorIndex = this._getMonitorIndexAtPointer();

        // Check if focusNodeWindow is on a different monitor than the pointer.
        // On Wayland, keyboard focus and pointer location can diverge, so we need
        // to rebase navigation on the pointer's monitor when they differ.
        if (focusNodeWindow && focusWindow) {
          try {
            const focusWindowMonitor = focusWindow.get_monitor();
            if (focusWindowMonitor !== pointerMonitorIndex) {
              // Pointer is on a different monitor than the focused window:
              // rebase navigation to the pointer monitor.
              Logger.info(
                `[Focus] Pointer monitor (${pointerMonitorIndex}) != focus window monitor (${focusWindowMonitor}), rebasing to monitor node`
              );
              focusNodeWindow = null;
            }
          } catch (e) {
            // If anything goes wrong, fall back to existing behavior
            Logger.warn(`[Focus] Failed to compare focus window monitor: ${e}`);
          }
        }

        // Fallback: If no usable focusNodeWindow (either none, or monitor mismatch),
        // start from the monitor node under the pointer.
        if (!focusNodeWindow) {
          const wsIndex = global.display.get_workspace_manager().get_active_workspace_index();
          const monWsId = `mo${pointerMonitorIndex}ws${wsIndex}`;

          // Find the Monitor Node directly
          focusNodeWindow = this.tree.findNode(monWsId);
          Logger.info(`[Focus] Using monitor node: ${monWsId}`);
        }
    ...
    if (this._freezeRender || !this.ext.settings.get_boolean("tiling-mode-enabled")) {
      ...
      if (!this._renderTreeSrcId) {
        this._renderTreeSrcId = GLib.idle_add(GLib.PRIORITY_DEFAULT, () => {
          ...
          // Persist tree state to JSON as the single source of truth
  ...
  /**
   * Persist the current tree state to JSON file.
   * This is the single source of truth for the layout persisted on disk.
   */
  persistTreeState() {
    try {
      const state = this.tree.toJSONTree();
      this.ext.configMgr.treeState = state;
    } catch (e) {
      Logger.warn(`Failed to persist tree state: ${e}`);
    }
  }
   ...
   *
   * Strategy:
   *  - When called with from === "enable":
   *      * Ignore any JSON snapshot and rebuild the tree from the current
   *        live Meta.Window state. This prevents re-applying a stale layout
   *        when the user simply toggles the extension off and back on.
   *  - For any other context:
   *      * First try to reload the tree from the JSON dump file (tree.json),
   *        which is the single source of truth for the persisted layout.
   *      * If that fails, fall back to rebuilding from live Meta.Window list.
   */
  reloadTree(from) {
    if (!this._reloadTreeSrcId) {
      this._reloadTreeSrcId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
        ...
        // Decide whether this caller should use JSON or not.
        // On "enable", we always adopt the live layout (no JSON).
        const shouldUseJson = from !== "enable";
        let reloadedFromJson = false;
        let windowsAllForRebind = null;

        if (shouldUseJson) {
          try {
            const state = this.ext.configMgr.treeState;
            if (state) {
              const windowsAll = this.windowsAllWorkspaces;
              reloadedFromJson = this.tree.reloadFromJson(state, windowsAll);
              if (reloadedFromJson) {
                Logger.info(`Reloaded tree from JSON state (from=${from})`);
                // Store windows for rebinding after tree restore
                windowsAllForRebind = windowsAll;
                // After JSON reload, track any windows not present in the snapshot
                // (newly opened windows, dialogs, etc.)
                this.trackCurrentWindows();
              }
            }
          } catch (e) {
            Logger.warn(`Failed to reload tree from JSON: ${e}`);
            reloadedFromJson = false;
          }
        }

        if (!reloadedFromJson) {
          // Either:
          //  - from === "enable", in which case we *intentionally* skip JSON; or
          //  - JSON was missing/invalid or reloadFromJson() returned false.
          //
          // In both cases, rebuild the tree from the current live window state.
          Logger.info(
            `Reloading tree from live state (${shouldUseJson ? "JSON reload failed/unavailable" : "enable"})`
          );

          // Clear existing layout under the root node
          this.tree.childNodes.length = 0;
          this.tree.attachNode = undefined;

          // Recreate workspace/monitor structures from GNOME state
          this.tree._initWorkspaces();

          // In the specific case of "enable", we want to *adopt*
          // the current layout instead of re-placing windows by pointer.
          const previousMode = this._adoptLiveLayoutMode;
          this._adoptLiveLayoutMode = (from === "enable");

          // Walk all current windows in all workspaces and attach them
          // according to their existing positions. This *adopts* the current
          // layout instead of forcing a stale one from disk.
          this.trackCurrentWindows();

          // Restore previous mode (should be false for all normal flows).
          this._adoptLiveLayoutMode = previousMode;
        }

        // If we restored from JSON, rebind window signals and borders.
        // This is necessary because _removeSignals() clears all per-window
        // state when the extension is disabled, and reloadFromJson() only
        // restores the tree structure, not the Forge hooks.
        if (reloadedFromJson && windowsAllForRebind) {
          this._rebindRestoredWindows(windowsAllForRebind);
        }

        // Render the tree and reapply decorations/borders. This will also
        // persist the tree state back to JSON via persistTreeState(), as
        // wired inside renderTree().
  ...
  /**
   * Attach per-window state (signals, borders) to a metaWindow and its node.
   * This is used both when tracking new windows and when rebinding windows
   * after re-enabling the extension with restore-layout-on-enable.
   *
   * @param {Meta.Window} metaWindow - The GNOME Shell window
   * @param {Node} nodeWindow - The corresponding Forge tree node
   */
  attachWindowState(metaWindow, nodeWindow) {
    if (!metaWindow || !nodeWindow) return;

    let windowActor = metaWindow.get_compositor_private();
    if (!windowActor) return;

    // Attach window signals (position, size, unmanaged, focus, workspace-changed)
    ...
    // Attach actor signals (destroy handler)
    ...
    // Create and attach the border widget
    ...
    // Post-process the window (center prefs window, etc.)
    this.postProcessWindow(nodeWindow);
  }

  /**
   * Rebind Forge state (signals, borders) to windows that were restored
   * from a JSON tree state. This is called after reloadFromJson() succeeds
   * to ensure windows are fully re-attached to the tiling system.
   *
   * @param {Meta.Window[]} windowsAll - Array of windows that were present during restore
   */
  _rebindRestoredWindows(windowsAll) {
    if (!windowsAll || windowsAll.length === 0) return;

    Logger.info(`[_rebindRestoredWindows] Rebinding ${windowsAll.length} windows`);

    for (const metaWindow of windowsAll) {
      // Skip windows that don't pass our validation
      if (!this._validWindow(metaWindow)) {
        Logger.debug(`[_rebindRestoredWindows] Skipping invalid window: ${metaWindow.get_title()}`);
        continue;
      }
      ...
      if (nodeWindow && nodeWindow.isWindow()) {
        Logger.debug(`[_rebindRestoredWindows] Rebinding window: ${metaWindow.get_title()}`);
        this.attachWindowState(metaWindow, nodeWindow);
      } else {
        Logger.debug(`[_rebindRestoredWindows] No node found for window: ${metaWindow.get_title()}`);
      }
    }

    // Ensure borders and decorations are consistent after rebinding
    this.updateDecorationLayout();
    this.updateBorderLayout();

    Logger.info(`[_rebindRestoredWindows] Rebinding complete`);
  }
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // Decide whether to use pointer-based placement or to *adopt*
        // the existing monitor/workspace of the window.
        const adoptLiveLayout = !!this._adoptLiveLayoutMode;

        let targetMonitor;
        let targetWorkspace;
        const focusMonitor = global.display.get_current_monitor();

        if (adoptLiveLayout) {
          // Adoption mode (used during reloadTree("enable") + live rebuild):
          // - Keep the window on its current monitor and workspace.
          const ws = metaWindow.get_workspace();
          targetMonitor = metaWindow.get_monitor();
          targetWorkspace = ws ? ws.index() : global.workspace_manager.get_active_workspace_index();

          Logger.info(`[trackWindow] (adopt-layout) Window: ${metaWindow.get_title()}`);
          Logger.info(`[trackWindow] (adopt-layout) Using existing monitor=${targetMonitor}, workspace=${targetWorkspace}`);
        } else {
          // Normal behaviour: use pointer-based monitor detection for proper Wayland support.
          // On Wayland, get_current_monitor() returns the monitor with keyboard focus,
          // not where the mouse is. This ensures windows spawn on the monitor
          // where the user's mouse is located.
          targetMonitor = this._getMonitorIndexAtPointer();
          targetWorkspace = global.display.get_workspace_manager().get_active_workspace_index();

          // Force the physical window to the monitor where the mouse is located.
          // This is required on Wayland because the OS defaults to opening windows 
          // on the currently focused monitor, regardless of pointer position.
          if (metaWindow.get_monitor() !== targetMonitor) {
            metaWindow.move_to_monitor(targetMonitor);
          }

          Logger.info(`[trackWindow] Window: ${metaWindow.get_title()}`);
          Logger.info(`[trackWindow] Pointer monitor: ${targetMonitor}, Focus monitor: ${focusMonitor}, Workspace: ${targetWorkspace}`);
        }

        let metaMonWs = `mo${targetMonitor}ws${targetWorkspace}`;
        Logger.info(`[trackWindow] Target node ID: ${metaMonWs}`);
        ...
        Logger.info(`[trackWindow] metaMonWsNode found: ${metaMonWsNode?.nodeValue}, hasWindows: ${hasWindows}, windowCount: ${windowNodes.length}`);

        // Get the current attach node (usually the last focused window)
        let treeAttachNode = this.tree.attachNode;
        Logger.info(`[trackWindow] tree.attachNode: ${treeAttachNode?.nodeValue}, type: ${treeAttachNode?.nodeType}`);

        attachTarget = treeAttachNode;
        attachTarget = attachTarget ? this.tree.findNode(attachTarget.nodeValue) : null;

        Logger.info(`[trackWindow] Initial attachTarget: ${attachTarget?.nodeValue}`);

        if (!attachTarget) {
          attachTarget = metaMonWsNode;
          Logger.info(`[trackWindow] No attachTarget, using metaMonWsNode: ${attachTarget?.nodeValue}`);
        } else {
          const containsAttachTarget = metaMonWsNode.contains(attachTarget);
          Logger.info(`[trackWindow] metaMonWsNode.contains(attachTarget): ${containsAttachTarget}`);
          if (hasWindows) {
            if (attachTarget && containsAttachTarget) {
              // Use the attach target - it's in the correct monitor
              Logger.info(`[trackWindow] Using attachTarget (in correct monitor): ${attachTarget?.nodeValue}`);
            } else {
              // Attach target is on different monitor, use first window on pointer monitor
              attachTarget = windowNodes[0];
              Logger.info(`[trackWindow] attachTarget on different monitor, using first window: ${attachTarget?.nodeValue}`);
            }
          } else {
            attachTarget = metaMonWsNode;
            Logger.info(`[trackWindow] No windows on pointer monitor, using metaMonWsNode: ${attachTarget?.nodeValue}`);
          }
        }

        Logger.info(`[trackWindow] Final attachTarget: ${attachTarget?.nodeValue}, type: ${attachTarget?.nodeType}`);

        // Attach the new window node under the chosen attachTarget
        ...
        // Mark pointer placement only in the normal (non-adopt) path,
        // so later logic that relies on pointer-placed windows behaves correctly.
        if (!adoptLiveLayout) {
          metaWindow._forgePointerPlaced = true;
          metaWindow._forgeTargetMonitor = targetMonitor;
          Logger.info(`[trackWindow] Marked window with _forgePointerPlaced=true, targetMonitor=${targetMonitor}`);
        } else {
          metaWindow._forgePointerPlaced = false;
          metaWindow._forgeTargetMonitor = targetMonitor;
          Logger.info(`[trackWindow] (adopt-layout) Respecting existing monitor/workspace, targetMonitor=${targetMonitor}`);
        }

        metaWindow.firstRender = true;

        // Attach signals, borders, and post-process via the shared helper
        this.attachWindowState(metaWindow, nodeWindow);
        ...
        this.queueEvent(
          {
            ...
            callback: () => {
              ...
              // Move pointer to center after window geometry is finalized
              this.movePointerWith(nodeWindow, { force: true, center: true });
            },
          },
          200
        );
  ...
  postProcessWindow(nodeWindow) {
    ...
    if (metaWindow) {
      ...
      // Pointer move now happens after window geometry is finalized in window-create-queue
    }
  }
  ...
  windowDestroy(actor) {
    ...
    this.queueEvent({
      ...
      callback: () => {
        ...
        // Remove focus from all windows by focusing the default window (stage/background)
        // This is the proper way to have "no window focused" on Wayland
        try {
          global.display.focus_default_window(global.display.get_current_time());
        } catch (e) {
          Logger.warn(`Failed to remove focus after window destruction: ${e}`);
        }
      },
    });
  }
  ...
  /**
   * Set the suspended state for the window manager.
   * When suspended (true), destructive monitor/workarea signals are ignored
   * to preserve the Tree state during suspend/lock.
   * When resumed (false), the preserved Tree state is forcibly re-applied.
   */
  setSuspended(status) {
    this._isSuspended = status;
    Logger.info(`[setSuspended] Suspend state: ${status}`);

    if (!status) {
      // Resuming from suspend - restore the preserved layout
      Logger.info(`[setSuspended] Resuming - restoring window layout from memory`);

      // Force the Tree to re-apply its preserved state to all windows
      // This overrides any chaos GNOME created during the suspend
      if (this.tree) {
        this.tree.apply(this.tree);
        this.renderTree("resume-from-suspend", true);
        Logger.info(`[setSuspended] Layout restoration complete`);
      }
    }
  }
   ...
   * properly
   */
  movePointerWith(nodeWindow, { force = false, center = false } = {}) {
    ...
    if (shouldWarp) {
      ...
      if (this.canMovePointerInsideNodeWindow(nodeWindow)) {
        if (center) {
          this.warpPointerToNodeWindowCenter(nodeWindow);
        } else {
  ...
  // Explicitly warp to the center of the window
  warpPointerToNodeWindowCenter(nodeWindow) {
    if (!nodeWindow || !nodeWindow._data) return;
    ...
    const cx = metaRect.x + Math.floor(metaRect.width / 2);
    const cy = metaRect.y + Math.floor(metaRect.height / 2);

    const seat = Clutter.get_default_backend().get_default_seat();
    if (seat) {
      const wmTitle = metaWindow.get_title();
      Logger.debug(`moved pointer (center) to [${wmTitle}] at (${cx},${cy})`);
      seat.warp_pointer(cx, cy);
    }
  }
  ...
  /**
   * Get the monitor index containing the current mouse pointer position.
   * This is essential for Wayland where get_current_monitor() returns the
   * monitor with keyboard focus, not where the mouse is located.
   *
   * @returns {number} Monitor index containing the pointer, or fallback to current monitor
   */
  _getMonitorIndexAtPointer() {
    const [pointerX, pointerY] = global.get_pointer();
    const numMonitors = global.display.get_n_monitors();
    const focusMonitor = global.display.get_current_monitor();

    Logger.info(`[Pointer Monitor Detection] Pointer at (${pointerX}, ${pointerY}), numMonitors=${numMonitors}, focusMonitor=${focusMonitor}`);

    for (let i = 0; i < numMonitors; i++) {
      const monitorGeometry = global.display.get_monitor_geometry(i);
      Logger.info(`[Pointer Monitor Detection] Monitor ${i}: x=${monitorGeometry.x}, y=${monitorGeometry.y}, w=${monitorGeometry.width}, h=${monitorGeometry.height}`);
      if (
        pointerX >= monitorGeometry.x &&
        pointerX < monitorGeometry.x + monitorGeometry.width &&
        pointerY >= monitorGeometry.y &&
        pointerY < monitorGeometry.y + monitorGeometry.height
      ) {
        Logger.info(`[Pointer Monitor Detection] Pointer is inside monitor ${i}`);
        return i;
      }
    }

    // Fallback: if pointer is not found in any monitor (edge case),
    // use the traditional get_current_monitor()
    Logger.info(`[Pointer Monitor Detection] Fallback to focusMonitor=${focusMonitor}`);
