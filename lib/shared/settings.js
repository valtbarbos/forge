...
export class ConfigManager extends GObject.Object {
  ...
  get defaultTreeConfigFile() {
    const defaultTreeConfig = GLib.build_filenamev([
      this.extensionPath,
      `config`,
      `tree.json`,
    ]);

    Logger.trace(`default-tree-config: ${defaultTreeConfig}`);
    const defaultTreeConfigFile = Gio.File.new_for_path(defaultTreeConfig);

    if (defaultTreeConfigFile.query_exists(null)) {
      return defaultTreeConfigFile;
    }
    return null;
  }
  ...
  get treeConfigFile() {
    ...
    const defaultSettingFile = this.defaultTreeConfigFile;
    return this.loadFile(profileSettingPath, settingFile, defaultSettingFile);
  }

  get treeState() {
    let treeConfigFile = this.treeConfigFile;
    let treeState = null;

    if (!treeConfigFile) {
      treeConfigFile = this.defaultTreeConfigFile;
    }
    ...
    try {
      let [success, contents] = treeConfigFile.load_contents(null);
      if (success) {
        const treeConfigContents = imports.byteArray.toString(contents);
        Logger.trace(`${treeConfigContents}`);
        treeState = JSON.parse(treeConfigContents);
      }
    } catch (e) {
      Logger.warn(`Failed to parse tree.json: ${e}`);
      treeState = null;
    }
    return treeState;
  }
  ...
  set treeState(state) {
    ...
    if (!treeConfigFile) {
      Logger.warn(`No tree.json or default tree.json; cannot persist tree state`);
      return;
    }
    ...
    try {
      if (
        GLib.mkdir_with_parents(treeConfigFile.get_parent().get_path(), PERMISSIONS_MODE) === 0
      ) {
        treeConfigFile.replace_contents(
          contents,
          null,
          false,
      ...
      Logger.warn(`Failed to write tree.json: ${e}`);
    }
  }
